...
int *Search(char *file,char *cs, char *pattern,char *command,char *writePath){
    ...
    for(unsigned int i = frls[0];i<=frls[1];i++){
        ...
        while(sb != '['){
            ...
            sb = getSymbol(file,realRank(1,file,".b",writePath,Sybfront));
        }
        //reverse the string
        inplace_reverse(id);
        if(atoi(id)>0){
...
char *findRecord(char *file,char *cs,char *identifier,char *writePath){
    ...
    unsigned int *fr_ls = backwardSearch(file,cst,identifier,writePath);
    // printf("aa%d %d\n",fr_ls[0],fr_ls[1]);
    ...
    for(unsigned int i = fr_ls[0]; i<=fr_ls[1];i++){
        char readID[20] = {'\0'};
        int tp = realRank(1,file,".b",writePath,i);
...
int main(int argc, char* argv[]){
    ...
    // printf("%u\n",realRank(1,readPath,".b",writePath,58));
